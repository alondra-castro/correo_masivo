...
<html lang="es">
  <head>
    ...
    <title>Enviar Aviso Masivo</title>
  </head>
  <body>
...
<?php
include('config.php');
  $QueryInmuebleDetalle = ("SELECT * FROM profile_user WHERE email !='' limit 50 ");
        ...
        <tbody>
          ...
          while ($dataClientes = mysqli_fetch_array($resultadoInmuebleDetalle)) { ?>
            <tr>
                <td>
                  <?php echo $i; ?>
                    <input type="checkbox"  name="email[]" class="CheckedAK" email="<?php echo $dataClientes['email']; ?>" value="<?php echo $dataClientes['email']; ?>"/>
                  </td>
                <td><?php echo $dataClientes['usu_usuario']; ?></td>
                <td><?php echo $dataClientes['email']; ?></td>
                <td>
                  <?php
